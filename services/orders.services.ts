...
export const getOneOrders = async (id: number) => {
  const endPoint = APP_END_POINTS.orders.single + "/" + id;
...
export const changeOrderStatus = async (dataSend: any, id: number) => {
  const endPoint = APP_END_POINTS.orders.changeStatus + "/" + id;
